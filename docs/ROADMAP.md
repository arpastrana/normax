...
> Then `validation/strut_gradients.py`: verify `jax.grad` of
...
> Add `validation/interaction_gradients.py`: gradcheck w.r.t. N_Ed, M_y, M_z and
...
All four steps ran. `validation/opensees_ddm.py`; full write-up in
...
**Scope:**
> `validation/handoff_forces.py`. Take the 2D funicular arch from
...
`validation/handoff_forces.py`, 29 cases in
...
it, and `validation/class_ratio_sweep.py` sweeps it across two class
...
- **`validation/class_ratio_sweep.py` lost a container.** `ClassBranch` paired
...
`analysis.backend`, `validation/pynite_adjoint.py`, and
...
Flip it with one keyword and rerun `validation/class_ratio_sweep.py`, which
