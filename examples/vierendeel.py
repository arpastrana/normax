...
def chord_signs(config: TrussConfig) -> tuple[np.ndarray, np.ndarray]:
    """
...
def signed_start(
    ...
    signs, chords = chord_signs(config.structure)
    guard = guard_signs(fit.q, signs, chords, config.subspace.margin_fraction)
    q = signed_shift(fit.q, fit.self_stresses[:, 0], guard)
...
def main(config_path: Path) -> None:
    ...
    report = Report()
    report.write_banner("Vierendeel truss — one search to a design")
    ...
    q_start, guard = signed_start(structure, basis, loads, config)
    guarded = guard if config.subspace.margin_fraction > 0.0 else None
    constraints = design_constraints(config.constraints, guarded)
    ...
    drawn.savefig(FIGURES / "vierendeel_designs.png", dpi=200)
    descended.savefig(FIGURES / "vierendeel_descent.png", dpi=200)
    written = [("figures", str(FIGURES)), ("data", str(DATA / "vierendeel.npz"))]
