...
the geometry moves. The end-to-end search moves the held-plan subspace
...
forces, the EN 1993-1-1 check into utilizations. The free-heights search drops
the form finder and hands the optimizer the node heights directly, driving
the same T2 and T3 alone. The sizing-only search holds the truss as drawn and
...
ceiling and above the sag floor. The flow lives in `normax.searches` and is
...
**On this truss the two shaped searches span the same geometries.** Experiment
...
search gives nothing away. Whether the two parametrizations also *land* on the
...
Run with `uv run --group pipeline --group viz python examples/warren.py
[warren.yaml]`.
"""
...
WARREN_PROFILE = StructureProfile(
    banner="Warren truss — three searches to a design",
    prefix="18_warren",
...
if __name__ == "__main__":
    ...
    requested = run_searches(
        WARREN_PROFILE,
        described or Path(__file__).with_name("warren.yaml"),
    )
