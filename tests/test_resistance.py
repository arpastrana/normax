...
def test_n_cr_matches_euler():
    ...
    assert force_critical(
        second_moment, buckling_length, Steel(e_mod=e_mod)
    ) == pytest.approx(expected)


def test_n_cr_scales_inversely_with_length_squared():
    assert force_critical(1e6, 8000.0, Steel()) == pytest.approx(
        0.25 * force_critical(1e6, 4000.0, Steel())
    )
...
def test_lambda_1_matches_the_93_9_epsilon_form():
    ...
    for f_y in (235.0, 275.0, 355.0, 420.0, 460.0):
        assert slenderness_reference(Steel(f_y=f_y)) == pytest.approx(
...
def test_slenderness_forms_agree(buckling_length):
    ...
    from_force = slenderness_from_force(
        area,
        Steel(f_y=f_y),
        force_critical(second_moment, buckling_length, Steel()),
    )
    from_gyration = slenderness_from_gyration(buckling_length, radius, Steel(f_y=f_y))
...
def test_slenderness_scales_with_length():
    ...
    short = slenderness_from_force(
        area, Steel(f_y=f_y), force_critical(second_moment, 4000.0, Steel())
    )
    long = slenderness_from_force(
        area, Steel(f_y=f_y), force_critical(second_moment, 8000.0, Steel())
    )
...
def test_n_c_rd_equals_n_pl_rd():
    ...
    assert resistance_compression(
        7367.0, Steel(f_y=355.0, gamma_m0=1.0)
    ) == pytest.approx(resistance_yielding(7367.0, Steel(f_y=355.0, gamma_m0=1.0)))


def test_n_t_rd_takes_the_smaller_resistance():
    gross = resistance_yielding(5000.0, Steel(f_y=265.0, gamma_m0=1.0))
    net = resistance_fracture(4406.0, Steel(f_u=430.0, gamma_m2=1.1))

    assert resistance_tension(
        5000.0, 4406.0, Steel(f_y=265.0, f_u=430.0, gamma_m0=1.0, gamma_m2=1.1)
...
def test_n_t_rd_is_governed_by_the_net_section_when_holes_are_large():
    ...
    assert (
        resistance_tension(
            5000.0, 2000.0, Steel(f_y=265.0, f_u=430.0, gamma_m0=1.0, gamma_m2=1.1)
        )
        < gross
    )
...
def test_n_b_rd_reduces_the_cross_section_resistance():
    resistance = resistance_compression(7367.0, Steel(f_y=355.0, gamma_m0=1.0))

    assert resistance_buckling(
        0.877915, 7367.0, Steel(f_y=355.0, gamma_m1=1.0)
...
def test_n_b_rd_meets_n_c_rd_for_a_stocky_member():
    ...
    assert resistance_buckling(
        1.0, 7367.0, Steel(f_y=355.0, gamma_m1=1.0)
    ) == pytest.approx(resistance_compression(7367.0, Steel(f_y=355.0, gamma_m0=1.0)))


def test_partial_factors_divide():
    assert resistance_compression(
        7367.0, Steel(f_y=355.0, gamma_m0=1.1)
    ) == pytest.approx(
        resistance_compression(7367.0, Steel(f_y=355.0, gamma_m0=1.0)) / 1.1
    )
...
def test_the_shear_resistance_follows_equation_6_18():
    ...
    assert resistance_shear(
        area_shear(AREA_CHS), Steel(f_y=355.0, gamma_m0=1.0)
...
def test_the_shear_resistance_of_the_fixture_section():
    ...
    resistance = resistance_shear(area_shear(AREA_CHS), Steel(f_y=355.0, gamma_m0=1.0))

    assert float(resistance) / 1e3 == pytest.approx(961.2, rel=1e-3)


def test_the_shear_resistance_divides_by_the_partial_factor():
    assert resistance_shear(4690.0, Steel(f_y=355.0, gamma_m0=1.1)) == pytest.approx(
        float(resistance_shear(4690.0, Steel(f_y=355.0, gamma_m0=1.0))) / 1.1
    )
...
def test_the_shear_yield_stress_is_the_tensile_one_over_root_three():
    ...
    assert float(
        resistance_shear(1.0, Steel(f_y=355.0, gamma_m0=1.0))
...
def test_the_shear_resistance_grows_with_the_area():
    ...
    assert jnp.all(
        jnp.diff(resistance_shear(area_shear(areas), Steel(f_y=355.0, gamma_m0=1.0)))
        > 0.0
    )
...
def test_resistances_are_float64():
    ...
    assert (
        resistance_compression(7367.0, Steel(f_y=355.0, gamma_m0=1.0)).dtype
        == jnp.float64
    )
